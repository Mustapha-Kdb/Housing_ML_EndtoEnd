...
def prepare_metros_join_key(metros_df: pd.DataFrame) -> pd.DataFrame:
    """Strip city code from metro_full to create join_key."""
    metros_df = metros_df.copy()
    metros_df["join_key"] = metros_df["metro_full"].str.split(',').str[0].str.strip()
    metros_df["join_key"] = metros_df["join_key"].apply(normalize_city)
    return metros_df
...
def clean_and_merge(df: pd.DataFrame, metros_path: str | None = "data/raw/usmetros.csv") -> pd.DataFrame:
    ...
    # Prepare metros with join_key (strip city code after comma)
    metros = prepare_metros_join_key(metros)
    
    # Merge using join_key
    df = df.merge(
        metros[["join_key", "metro_full", "lat", "lng"]],
        how="left",
        left_on="city_full",
        right_on="join_key"
    )
    ...
    df.drop(columns=["join_key", "metro_full"], inplace=True, errors="ignore")
